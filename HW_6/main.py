...
@app.get("/")
def root():
    return {"Message": "Welcome to internet-shop!"}


@app.get("/fake_users/{count}")
async def create_note(count: int):
    for i in range(count):
        query = db.users.insert().values(name=f'user{i}', surname=f'surname{i}', email=f'email00{i}@example.com',
                                         password=f'defaultpassword{i}')
...
async def create_note(count: int):
    for i in range(count):
        query = db.products.insert().values(title=f'product {i}', description=f'Product {i} description',
...
# Получение всех users/products/orders
...
# Получкние одного user/product/order
